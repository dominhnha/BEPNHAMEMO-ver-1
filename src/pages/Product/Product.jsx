...
const Product = props => {
   ...
   const [Classify,SetClassify] = useState("NameProduct")
  //  ------------------------Post------------------------
  ...
  // search event 
  const handleSearch = async (queryText,name,proviso,Classify) => {
    try{
      // const data = await searchProduct(queryText,name,proviso,Classify);
                            ...
                            listClassify.map(item=>{
                              return (
                                <li 
                                  ...
                                  className="Product__opinion__item"
                                  key={v4()}
                                  onClick={(()=>handleclassify(`${item.payload}`,Sort.field,Sort.payload))}
                                >{item.payload}</li>
                              )
                            })
                          }
